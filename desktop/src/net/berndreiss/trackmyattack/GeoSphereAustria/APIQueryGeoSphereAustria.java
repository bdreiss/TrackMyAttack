...
public class APIQueryGeoSphereAustria implements APIQuery {
    ...
    /**
     * Creates a new instance and initializes list of GeoSphere Austria weather
     * stations.
     */
    ...
    public void query(LocalDate startDate, LocalDate endDate, DataWrapper geoData, Category category) {
        ...
        while (!(currentDate.isAfter(endDate))) {
            ...
            if (newStation != null) {
                ...
                if (currentStation == null || newStation.compareTo(currentStation) != 0) {
    ...
    public void parseJSON(String jsonString, Category category) {
        ...
        // get JSON strings for Druck, Temp etc. and extractData resulting in
        // ArrayList<Float>
        ...
        for (int i = 0; i < timestamps.size(); i++) {
            ...
            // TODO: handle case of data not being complete: first case all values are null
            // -> remove station and make new query (make function boolean?), make
            // nearestStation take List of Nonos -> make inner class query

            if (temperatureMedian != null)
                geoData.addDatumDirectly(category, GeoDataType.TEMPERATURE_MEDIAN.toString(),
                        new GeoDatum(date.atStartOfDay(), temperatureMedian, GeoDataType.TEMPERATURE_MEDIAN));
            if (temperatureMin != null)
                geoData.addDatumDirectly(category, GeoDataType.TEMPERATURE_MIN.toString(),
                        new GeoDatum(date.atStartOfDay(), temperatureMin, GeoDataType.TEMPERATURE_MIN));
            if (temperatureMax != null)
                geoData.addDatumDirectly(category, GeoDataType.TEMPERATURE_MAX.toString(),
                        new GeoDatum(date.atStartOfDay(), temperatureMax, GeoDataType.TEMPERATURE_MAX));
            if (druck != null)
    ...
    private static String JSONQuery(LocalDate startDate, LocalDate endDate, GeoSphereAustriaStation station) {
        ...
        jsonString = JSONQuery(
                PREFIX_AUSTRIA + startDate + INFIX1_AUSTRIA + endDate + INFIX2_AUSTRIA + station.getId());
        return jsonString;
    }
    ...
    private static void initializeStations() {
        ...
        // at the moment sorting is not necessary, since getNearestStationAustria()
    ...
    private static GeoSphereAustriaStation getNearestStationAustria(Coordinate coordinates) {
        ...
        for (Station s : stations) {
            ...
            Double newDistance = Math.abs((coordinates.getLatitude() - lat))
                    + Math.abs((coordinates.getLongitude() - lon));
